...
<%
	...
	//if(id!=null && pw!=null && id.trim().length()>2 && pw.trim().length()>3){
	if(id!=null && pw!=null){
		try{
	...
	if(reid!=null && reid.equals("on")){
		// 쿠키 저장 : 사용자 ID를 저장
		response.addCookie(CookieBox.makeCookie("reid", id, "/", 60*60*24*365));
	} else {
		// 쿠키값을 저장하지 않는다!!!
		response.addCookie(CookieBox.makeCookie("reid", id, "/", 0));
	}
